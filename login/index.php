...
if (isLoggedIn()) {
    $redirect = sanitize($_GET['redirect'] ?? '/produk');
    // Pastikan redirect hanya ke path internal
    if (!str_starts_with($redirect, '/')) $redirect = '/produk';
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .login-container{flex:1;display:flex;align-items:center;justify-content:center;padding:40px 24px;}
  .login-box{background:#fff;border:1px solid var(--line);border-radius:20px;padding:40px;max-width:420px;width:100%;box-shadow:0 4px 6px rgba(0,0,0,0.05);}
  .login-box h1{font-size:28px;margin-bottom:8px;text-align:center;}
  .login-box .subtitle{text-align:center;color:#666;font-size:14px;margin-bottom:32px;}
  .form-group{margin-bottom:20px;}
  .form-group label{display:block;font-weight:600;font-size:14px;margin-bottom:8px;}
  .form-group input{width:100%;padding:12px 16px;border:1px solid var(--line);border-radius:10px;font-family:inherit;font-size:14px;background:#fff;}
  .form-group input:focus{outline:none;border-color:var(--red);}
  .btn-login{width:100%;padding:14px;background:var(--red);color:#fff;border:none;border-radius:10px;font-weight:700;font-size:15px;cursor:pointer;font-family:inherit;}
  .btn-login:hover{background:#b81f24;}
  ...
  .demo-credentials{background:#f5f5f5;border:1px solid var(--line);border-radius:10px;padding:16px;margin-bottom:24px;font-size:13px;}
  .demo-credentials h4{font-size:14px;margin-bottom:8px;color:var(--red);}
  .demo-credentials code{background:#fff;padding:2px 6px;border-radius:4px;font-family:'Courier New',monospace;}
...
<div class="login-container">
  <div class="login-box">
    <h1>Masuk</h1>
    <p class="subtitle">Belanja lebih mudah dengan akun Klik Madura</p>
    ...
    <div class="register-link">Belum punya akun? <a href="/register">Daftar sekarang</a></div>
  </div>
</div>
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Merge localStorage cart ke server jika ada
        const localCart = JSON.parse(localStorage.getItem('klikMaduraCart') || '[]');
        if (localCart.length > 0) {
          const mergeItems = localCart.map(i => ({ product_id: i.id, qty: i.qty }));
          await fetch('/api/cart.php', {
            method:  'POST',
            headers: { 'Content-Type': 'application/json' },
            body:    JSON.stringify({ action: 'merge', items: mergeItems }),
          }).catch(() => {});
          localStorage.removeItem('klikMaduraCart');
        }

        setTimeout(() => {
          const dest = redirect.startsWith('/') ? redirect : '/produk';
